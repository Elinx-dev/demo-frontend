...
const STATUS_COLOR: Record<string, { bg: string; color: string; label: string }> = {
  pending_maker: { bg: "#fff3e0", color: "#b8722e", label: "Pending Reg. Officer" },
  pending_checker: { bg: "#e8f4ff", color: "#1d4670", label: "Pending VAO" },
  pending_tahsildar: { bg: "#f3e8ff", color: "#7c3aed", label: "Pending Thasildar" },
...
export default function TahsildarVerificationDetailPage() {
  ...
  const loadItem = useCallback(async () => {
    ...
    try {
      const items = await slateApi.tahsildar.getQueue();
      const found = items.find((i) => i.txnId === txnId || (i as unknown as { id: string }).id === txnId);
  ...
  useEffect(() => {
    const ulpin = item?.ulpin ?? (item as unknown as { ulpin?: string } | null)?.ulpin;
    if (!ulpin) return;
  ...
  const handleApprove = async () => {
    ...
    setError("");
    try {
      await slateApi.tahsildar.approveAndGeneratePatta(item.id);
    } catch {
      try {
        await slateApi.tahsildar.approveMutation(item.id);
      } catch {
        /* best-effort */
      }
    }
  ...
  const handleRejectSubmit = async () => {
    ...
    setRejectBusy(true);
    try {
      await slateApi.tahsildar.rejectDeed(item.id, rejectReason.trim());
    } catch {
      try {
        await slateApi.tahsildar.rejectMutation(item.id, rejectReason.trim());
      } catch {
        /* best-effort */
      }
    } finally {
    ...
    navigate(SLATE_PATHS.TAHSILDAR_VERIFICATION_QUEUE);
  };

  const canApprove = item?.status === "pending_tahsildar";
      ...
      ? [
          {
            ...
            content: (
              <Card>
                ...
                >
                  {canApprove && !pattaGenerated && (
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      >
                        ...
                        {busy ? "Processing…" : "Approve and Generate Revenue Document"}
                      </Button>
                      <Button
                        variant="outline"
                  ...
                  {pattaGenerated && (
                    <div style={{ display: "flex", gap: 8 }}>
                      ...
                      >
                        <Eye size={14} /> Preview Revenue Document
                      </Button>
                      <Button
                        variant="outline"
                ...
                >
                  <AlertTriangle size={12} style={{ display: "inline", marginRight: 6, color: "#e85d04" }} />
                  <strong>Revenue Mutation:</strong> Approval executes on-chain ownership transfer
                  and initiates Revenue Document mutation. This action is irreversible. Verify all documents
                  before approving.
                </div>
              </Card>
            ),
    ...
    <div className="flex flex-col" style={{ height: "100%" }}>
      ...
      {loadingToken ? (
        ...
        <div style={{ flex: 1, minHeight: 0, marginTop: 8 }}>
          <PropertySplitView token={token} accentColor="#e85d04" extraLeftTabs={deedTab} />
        </div>
      ) : (
        <div
          style={{
      ...
      <Modal
        ...
        title="Revenue Document Preview"
        size="5xl"
      >
      ...
      <Modal
        ...
        title="Reject - Revenue Mutation"
        size="md"
      >
        <div style={{ marginBottom: 14, fontSize: 12.5, color: "#545c66", lineHeight: 1.6 }}>
          Rejecting this transaction will notify the citizen, VAO officer, Surveyor, and Registration
          Officer. The transaction will be returned for correction.
        </div>
        <textarea
          value={rejectReason}
